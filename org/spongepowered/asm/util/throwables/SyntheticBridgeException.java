...
/*     */ 
/*     */     
/*  68 */     Problem(String message) { this.message = message; }
/*     */ 
/*     */     
...
/*     */ 
/*     */     
/*  77 */     private static String getInsnName(AbstractInsnNode node) { return (node instanceof MethodInsnNode) ? ((MethodInsnNode)node).name : ""; }
/*     */ 
/*     */ 
/*     */     
/*  81 */     private static String getInsnDesc(AbstractInsnNode node) { return (node instanceof MethodInsnNode) ? ((MethodInsnNode)node).desc : ""; }
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 126 */     String merged = (String)Annotations.getValue(Annotations.getVisible(mda, org.spongepowered.asm.mixin.transformer.meta.MixinMerged.class), "mixin");
...
/* 136 */       printer.kv((index == this.index) ? ">>>>" : "", Bytecode.describeNode((AbstractInsnNode)iter.next()));
/*     */     }
/* 138 */     return printer.add(); } private PrettyPrinter printProblem(PrettyPrinter printer, IMixinContext context, MethodNode mda, MethodNode mdb) { int i; Type rtb, rta, argb[], arga[]; MethodInsnNode mdnb, mdna; Type tb;
/*     */     int index;
/*     */     Type ta, argsb[], argsa[];
/*     */     ListIterator<AbstractInsnNode> ib, ia;
...
/* 165 */           AbstractInsnNode na = (AbstractInsnNode)ia.next();
/* 166 */           AbstractInsnNode nb = (AbstractInsnNode)ib.next();
...
/*     */             
/* 171 */             Type ta = (va.var > 0) ? argsa[va.var - 1] : target;
/* 172 */             Type tb = (vb.var > 0) ? argsb[vb.var - 1] : target;
/* 173 */             printer.kv("Target " + index, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName(va), Integer.valueOf(va.var), ta });
/* 174 */             printer.kv("Incoming " + index, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName(vb), Integer.valueOf(vb.var), tb });
/*     */             
/* 176 */             if (ta.equals(tb)) {
/* 177 */               printer.kv("", "Types match: %s", new Object[] { ta });
/* 178 */             } else if (ta.getSort() != tb.getSort()) {
/* 179 */               printer.kv("", "Types are incompatible");
/* 180 */             } else if (ta.getSort() == 10) {
/* 181 */               ClassInfo superClass = ClassInfo.getCommonSuperClassOrInterface(ta, tb);
...
/* Location:              D:\工程开发\Cracked\phosphor-1.12.2-0.2.6+build50-universal.jar!\org\spongepowered\as\\util\throwables\SyntheticBridgeException.class
