...
/* 173 */       this.frameSize = Bytecode.getFirstNonArgLocalIndex(target.arguments, !this$0.isStatic());
/* 174 */       List<String> argNames = null;
/*     */       
/* 176 */       if (locals != null) {
/* 177 */         int baseArgIndex = this$0.isStatic() ? 0 : 1;
...
/* 194 */       this.argNames = (argNames != null) ? (String[])argNames.toArray(new String[argNames.size()]) : null;
...
/*     */ 
/*     */     
/* 212 */     private boolean isValueReturnOpcode(int opcode) { return (opcode >= 172 && opcode < 177); }
/*     */ 
/*     */ 
/*     */     
/* 216 */     String getDescriptor() { return this.canCaptureLocals ? this.descl : this.desc; }
/*     */ 
/*     */ 
/*     */     
/* 220 */     String getDescriptorWithAllLocals() { return this.target.getCallbackDescriptor(true, this.localTypes, this.target.arguments, this.frameSize, 32767); }
/*     */ 
/*     */ 
/*     */     
/* 224 */     String getCallbackInfoConstructorDescriptor() { return this.isAtReturn ? CallbackInfo.getConstructorDescriptor(this.target.returnType) : CallbackInfo.getConstructorDescriptor(); }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */     
/* 236 */     void add(AbstractInsnNode insn, boolean ctorStack, boolean invokeStack) { add(insn, ctorStack, invokeStack, false); }
/*     */ 
/*     */     
...
/*     */           }
/*     */           
/* 285 */           if (Annotations.getInvisibleParameter(this.handler, org.spongepowered.asm.mixin.injection.Coerce.class, arg) == null) {
/* 286 */             return false;
/*     */           }
/*     */           
...
/*     */ 
/*     */     
/* 302 */     boolean captureArgs() { return this.captureArgs; }
/*     */ 
/*     */     
/*     */     int marshalVar() {
...
/*     */ 
/*     */   
/* 333 */   private final Map<Integer, String> ids = new HashMap();
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/* 375 */         if (!injectionPoint.getClass().equals(org.spongepowered.asm.mixin.injection.points.BeforeReturn.class)) {
...
/*     */       }
/*     */       
/* 398 */       String existingId = (String)this.ids.get(Integer.valueOf(injectionNode.getId()));
...
/* 457 */         if (surrogateHandler != null && Annotations.getVisible(surrogateHandler, org.spongepowered.asm.mixin.injection.Surrogate.class) != null) {
/*     */           
...
/* 491 */         if (surrogateHandler != null && Annotations.getVisible(surrogateHandler, org.spongepowered.asm.mixin.injection.Surrogate.class) != null) {
/*     */           
...
/* 539 */     insns.add(new TypeInsnNode(187, errorClass));
/* 540 */     insns.add(new InsnNode(89));
/* 541 */     insns.add(new LdcInsnNode(message));
/* 542 */     insns.add(new MethodInsnNode(183, errorClass, "<init>", "(Ljava/lang/String;)V", false));
/* 543 */     insns.add(new InsnNode(191));
/* 544 */     return method;
/*     */   }
/*     */ 
/*     */ 
...
/*     */         } else {
/* 575 */           boolean isTop = (l > 0 && callback.localTypes[l - true] != null && callback.localTypes[l - 1].getSize() > 1);
...
/* 614 */       callback.add(new VarInsnNode(25, this.callbackInfoVar), false, true);
/*     */     } else {
...
/*     */     }
/*     */     
/* 633 */     callback.add(new InsnNode(89));
/* 634 */     callback.add(new VarInsnNode(callback.target.returnType.getOpcode(54), callback.marshalVar()));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*     */     
/* 654 */     callback.add(new TypeInsnNode(187, this.callbackInfoClass), true, !store, head);
/* 655 */     callback.add(new InsnNode(89), true, true, head);
/* 656 */     callback.add(new LdcInsnNode(id), true, !store, head);
/* 657 */     callback.add(new InsnNode(this.cancellable ? 4 : 3), true, !store, head);
/*     */     
/* 659 */     if (callback.isAtReturn) {
/* 660 */       callback.add(new VarInsnNode(callback.target.returnType.getOpcode(21), callback.marshalVar()), true, !store);
/* 661 */       callback.add(new MethodInsnNode(183, this.callbackInfoClass, "<init>", desc, false));
/*     */     } else {
/*     */       
/* 664 */       callback.add(new MethodInsnNode(183, this.callbackInfoClass, "<init>", desc, false), false, false, head);
/*     */     } 
/*     */ 
/*     */     
/* 668 */     if (store) {
/* 669 */       callback.target.addLocalVariable(this.callbackInfoVar, "callbackInfo" + this.callbackInfoVar, "L" + this.callbackInfoClass + ";");
/* 670 */       callback.add(new VarInsnNode(58, this.callbackInfoVar), false, false, head);
/*     */     } 
/*     */   }
/*     */ 
/*     */ 
...
/* 680 */       callback.add(new VarInsnNode(25, 0), false, true);
/*     */     }
/*     */ 
/*     */     
...
/* 710 */     String locationId = (String)this.ids.get(Integer.valueOf(callback.node.getId()));
...
/*     */     }
/*     */     
/* 724 */     callback.add(new VarInsnNode(25, this.callbackInfoVar));
/* 725 */     callback.add(new MethodInsnNode(182, this.callbackInfoClass, CallbackInfo.getIsCancelledMethodName(), 
/* 726 */           CallbackInfo.getIsCancelledMethodSig(), false));
/*     */     
/* 728 */     LabelNode notCancelled = new LabelNode();
/* 729 */     callback.add(new JumpInsnNode(153, notCancelled));
/*     */ 
/*     */ 
/*     */     
/* 733 */     injectReturnCode(callback);
/*     */     
/* 735 */     callback.add(notCancelled);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*     */       
/* 746 */       callback.add(new InsnNode(177));
/*     */     }
/*     */     else {
/*     */       
/* 750 */       callback.add(new VarInsnNode(25, callback.marshalVar()));
...
/* 753 */       callback.add(new MethodInsnNode(182, this.callbackInfoClass, accessor, descriptor, false));
/* 754 */       if (callback.target.returnType.getSort() == 10) {
/* 755 */         callback.add(new TypeInsnNode(192, callback.target.returnType.getInternalName()));
/*     */       }
/* 757 */       callback.add(new InsnNode(callback.target.returnType.getOpcode(172)));
/*     */     } 
/*     */   }
/*     */ 
/*     */ 
...
/*     */ 
/*     */   
/* 767 */   protected boolean isStatic() { return this.isStatic; }
/*     */ 
/*     */ 
/*     */   
/* 771 */   private static List<String> summariseLocals(String desc, int pos) { return summariseLocals(Type.getArgumentTypes(desc), pos); }
/*     */ 
/*     */   
...
/*     */ 
/*     */   
/* 787 */   static String meltSnowman(int index, String varName) { return (varName != null && '☃' == varName.charAt(0)) ? ("var" + index) : varName; }
/*     */ }


/* Location:              D:\工程开发\Cracked\phosphor-1.12.2-0.2.6+build50-universal.jar!\org\spongepowered\asm\mixin\injection\callback\CallbackInjector.class
