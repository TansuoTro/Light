...
/*      */   
/*  168 */   public ClassReader(byte[] b) { this(b, 0, b.length); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  194 */       int size, size, size, size, size; this.items[i] = index + 1;
/*      */       
/*  196 */       switch (b[index]) {
/*      */         case 3:
/*      */         case 4:
...
/*      */ 
/*      */   
/*  244 */   public int getAccess() { return readUnsignedShort(this.header); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*  256 */   public String getClassName() { return readClass(this.header + 2, new char[this.maxStringLength]); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*  270 */   public String getSuperName() { return readClass(this.header + 4, new char[this.maxStringLength]); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  308 */       String s; int fieldOrMethodRef, nameType, nameType, nameType, index = this.items[i];
...
/*      */ 
/*      */   
/*  439 */   public ClassReader(InputStream is) throws IOException { this(readClass(is, false)); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*  521 */   public void accept(ClassVisitor classVisitor, int flags) { accept(classVisitor, new Attribute[0], flags); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  620 */         for (int j = 0, v = u + 10; j < bootstrapMethods.length; j++) {
...
/*      */     }
/*  673 */     if (itanns != 0) {
/*  674 */       for (int i = readUnsignedShort(itanns), v = itanns + 2; i > 0; i--) {
...
/*      */     
/*  790 */     if (anns != 0) {
/*  791 */       for (int i = readUnsignedShort(anns), v = anns + 2; i > 0; i--) {
/*  792 */         v = readAnnotationValues(v + 2, c, true, fv
/*  793 */             .visitAnnotation(readUTF8(v, c), true));
/*      */       }
/*      */     }
/*  796 */     if (ianns != 0) {
/*  797 */       for (int i = readUnsignedShort(ianns), v = ianns + 2; i > 0; i--) {
/*  798 */         v = readAnnotationValues(v + 2, c, true, fv
/*  799 */             .visitAnnotation(readUTF8(v, c), false));
/*      */       }
/*      */     }
/*  802 */     if (tanns != 0) {
/*  803 */       for (int i = readUnsignedShort(tanns), v = tanns + 2; i > 0; i--) {
...
/*      */     }
/*  810 */     if (itanns != 0) {
/*  811 */       for (int i = readUnsignedShort(itanns), v = itanns + 2; i > 0; i--) {
...
/*  973 */       int i; int v; for (i = this.b[methodParameters] & 0xFF, v = methodParameters + 1; i > 0; i--, v += 4) {
...
/*      */     } 
/*  986 */     if (anns != 0) {
/*  987 */       for (int i = readUnsignedShort(anns), v = anns + 2; i > 0; i--) {
/*  988 */         v = readAnnotationValues(v + 2, c, true, mv
/*  989 */             .visitAnnotation(readUTF8(v, c), true));
/*      */       }
/*      */     }
/*  992 */     if (ianns != 0) {
/*  993 */       for (int i = readUnsignedShort(ianns), v = ianns + 2; i > 0; i--) {
/*  994 */         v = readAnnotationValues(v + 2, c, true, mv
/*  995 */             .visitAnnotation(readUTF8(v, c), false));
/*      */       }
/*      */     }
/*  998 */     if (tanns != 0) {
/*  999 */       for (int i = readUnsignedShort(tanns), v = tanns + 2; i > 0; i--) {
...
/*      */     }
/* 1006 */     if (itanns != 0) {
/* 1007 */       for (int i = readUnsignedShort(itanns), v = itanns + 2; i > 0; i--) {
...
/* 1066 */       int i, i, offset = u - codeStart;
...
/* 1098 */           for (i = readInt(u + 8) - readInt(u + 4) + 1; i > 0; i--) {
...
/* 1170 */           varTable = u + 8;
/* 1171 */           for (int j = readUnsignedShort(u + 8), v = u; j > 0; j--) {
...
/* 1187 */           for (int j = readUnsignedShort(u + 8), v = u; j > 0; j--) {
...
/* 1324 */       String idesc; int i; String iname; int i; String idesc; Object[] bsmArgs; int i; Label[] table; int bsmArgCount; String iname; Label[] values; Handle bsm; int max, keys[]; String iowner; int min, len, bsmIndex; boolean itf; int cpIndex, label, label, cpIndex; Label target; int offset = u - codeStart;
/*      */ 
/*      */       
...
/*      */           } else {
/* 1404 */             opcode = (opcode <= 166) ? ((opcode + 1 ^ true) - 1) : (opcode ^ true);
/*      */             
...
/* 1571 */         for (int i = typeTable.length; i > 0; ) {
...
/*      */ 
/*      */         
/* 2234 */         return v;case 1: frame[index] = Opcodes.INTEGER; return v;case 2: frame[index] = Opcodes.FLOAT; return v;case 3: frame[index] = Opcodes.DOUBLE; return v;case 4: frame[index] = Opcodes.LONG; return v;case 5: frame[index] = Opcodes.NULL; return v;case 6: frame[index] = Opcodes.UNINITIALIZED_THIS; return v;
/*      */       case 7:
...
/*      */ 
/*      */   
/* 2337 */   public int getItemCount() { return this.items.length; }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/* 2351 */   public int getItem(int item) { return this.items[item]; }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/* 2362 */   public int getMaxStringLength() { return this.maxStringLength; }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/* 2375 */   public int readByte(int index) { return this.b[index] & 0xFF; }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/* 2528 */   public String readClass(int index, char[] buf) { return readUTF8(this.items[readUnsignedShort(index)], buf); }
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/* Location:              D:\工程开发\Cracked\phosphor-1.12.2-0.2.6+build50-universal.jar!\org\spongepowered\asm\lib\ClassReader.class
